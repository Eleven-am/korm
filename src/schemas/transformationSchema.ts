...
export const aggregateTransformationSchema = functionCall(AggregateFunction).merge(transformationBase(TransformType.AGGREGATE))
    .merge(z.object({
        window: z.union([windowReferenceSchema, windowDefinitionSchema]).optional(),
    }));

export const windowFunctionTransformationSchema = functionCall(WindowFunction).merge(transformationBase(TransformType.WINDOW))
    .merge(z.object({
        over: z.object({
            alias: identifierSchema.optional(),
